@extends('layouts.app', ['activePage' => 'servicing', 'titlePage' => __('Update Servicing Details')])
...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ route('servicing.update', $servicing->id) }}" autocomplete="on" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="card ">
                            <div class="card-header card-header-primary">
                                <h4 class="card-title">{{ __('Item Servicing') }}</h4>
                                <p class="card-category">{{ __('Servicing information for '.$servicing->item->name) }}</p>
                            </div>
                            <div class="card-body ">
                                @if (session('status'))
                                    <div class="row">
                                        ...
                                        <div class="form-group{{ $errors->has('category') ? ' has-danger' : '' }}">
                                            <select class="selectpicker" data-style="btn btn-primary btn-round"  id="input-category"
                                                ...
                                                @forelse($categories as $category)
                                                    <option {{ $servicing->item->category_id == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->name }}</option>
                                                @empty
                                                @endforelse
                                            </select>
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('item') ? ' has-danger' : '' }}">
                                            <select class="selectpicker" data-style="btn btn-primary btn-round" id="input-item"
                                                ...
                                                <option selected value="{{ $servicing->item->id }}">{{ $servicing->item->name }}</option>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('problem') ? ' has-danger' : '' }}">
                                            <textarea name="problem" id="" cols="30" rows="15">{!! $servicing->problem !!}</textarea>
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('given_date') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('given_date') ? ' is-invalid' : '' }}" min="0"
                                                   name="given_date" id="input-given_date" type="date" placeholder="{{ __('Given Date') }}" value="{{ $servicing->given_date }}" required="true" aria-required="true"/>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('phone_number') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('phone_number') ? ' is-invalid' : '' }}" name="phone_number" id="input-phone_number" type="text" placeholder="{{ __('Center Phone Number') }}" value="{{ $servicing->phone_number }}" required="true" aria-required="true"/>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('location') ? ' has-danger' : '' }}">
                                            <textarea name="location" id="" cols="30" rows="15">{!! $servicing->location !!}</textarea>
